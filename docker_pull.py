...
# define you private docker repository
username = None
password = None
...
def get_auth_head(type):
    if registry != 'registry-1.docker.io':
        if username is None or password is None:
            print("Error: Username or password not defined.")
            return None
...
if (resp.status_code != 200):
    ...
    exit(1)
    # 对响应数据进行解析 根据平台拉取对应的镜像
    # 单镜像直接获取到layers
if 'layers' in resp.json():
    layers = resp.json()['layers']
    # 组合镜像获取到多平台信息
else:
    print('[+] This image has multiple platforms, fetching platform-specific manifests...')
    auth_head = get_auth_head('application/vnd.docker.distribution.manifest.list.v2+json')
    resp = requests.get('https://{}/v2/{}/manifests/{}'.format(registry, repository, tag), headers=auth_head,
                        verify=False)
    ...
    for manifest in manifests:
        for key, value in manifest["platform"].items():
            if value != platform[key]:
                continue
        auth_head = get_auth_head('application/vnd.docker.distribution.manifest.v2+json')
        resp = requests.get('https://{}/v2/{}/manifests/{}'.format(registry, repository, manifest["digest"]),
                            headers=auth_head, verify=False)

        if resp.status_code != 200:
            print('[-] Cannot fetch manifest for {} with digest {} [HTTP {}]'.format(repository, manifest["digest"],
                                                                                     resp.status_code))
            continue
...
if not layers:
    print('[-] Cannot find layers for {}:{} [HTTP {}]'.format(repository, tag, resp.status_code))
    exit(1)
